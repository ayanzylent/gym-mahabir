...
import { APP_NAME, WHATSAPP_LINK } from "@/config/constant";
      ...
      <nav className="fixed top-0 left-0 right-0 z-[60] py-4 px-4 md:px-12 flex justify-between items-center bg-[#070707] border-b border-white/5">
        ...
        <Link href="/demo" className="flex items-center gap-2" onClick={() => setIsMobileMenuOpen(false)}>
          <div className="relative flex items-center justify-center w-10 h-10 md:w-12 md:h-12">
            <Image src={'/logo.svg'} alt="logo" height={100} width={100} className="w-full h-full object-contain" />
          </div>
          <span className="text-xl md:text-2xl font-bold tracking-tight text-white font-sans">{APP_NAME}</span>
        </Link>
        ...
        <div className="flex items-center gap-2 md:gap-4 lg:gap-8">
          ...
          <div className="flex flex-row gap-2 items-center">
            ...
            <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="hidden md:flex items-center justify-center h-10 w-[120px] rounded-full bg-[#FF2A5F] hover:bg-rose-500 transition-colors shadow-[0_0_15px_rgba(255,42,95,0.4)] text-xs font-bold text-white tracking-widest">
              JOIN NOW
            </a>
          </div>
        </div>
      ...
      >
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="mt-4 pt-6 border-t border-white/10">
            <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="w-full flex justify-center py-4 rounded-full bg-[#FF2A5F] hover:bg-rose-500 transition-colors shadow-[0_4px_15px_rgba(255,42,95,0.4)] text-sm font-bold text-white tracking-widest uppercase">
              JOIN NOW
            </a>
          </div>
        </div>
