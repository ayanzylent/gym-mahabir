...
import { HeroTextAnimation } from "../custom/hero-text-animation";
import { WHATSAPP_LINK } from "@/config/constant";
    ...
    <section className="relative w-full aspect-[5/4] md:aspect-[4/3] lg:aspect-auto lg:min-h-[calc(100vh-72px)] bg-[#050505] overflow-hidden flex items-center">
      ...
      <div className="relative z-10 w-full px-6 md:px-12 lg:px-16 xl:px-24 flex flex-col justify-center">
        <div className="w-[85%] md:w-[70%] lg:w-[60%] xl:w-[50%]">
          ...
          <h1 className="text-3xl md:text-[40px] lg:text-6xl xl:text-[85px] leading-[1.1] md:leading-tight lg:leading-none font-black text-white tracking-[-0.03em] mb-4 md:mb-6 xl:mb-8 drop-shadow-xl relative z-10 min-h-[110px] sm:min-h-[110px] md:min-h-[130px] lg:min-h-[150px] xl:min-h-[200px]">
            Transform <HeroTextAnimation />
          </h1>
          <p className="text-gray-300 lg:text-[#e0e0e0] text-xs md:text-sm lg:text-base xl:text-xl font-light mb-6 md:mb-8 xl:mb-10 leading-relaxed max-w-[250px] md:max-w-md xl:max-w-[500px] drop-shadow-md">
            Our expert coaches bring the focus and discipline you need to perfect your form and maximize your gains.
          </p>

          <div className="flex flex-row items-center gap-4 md:gap-6 xl:gap-8">
            <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center cursor-pointer px-6 py-2.5 md:px-8 md:py-4 xl:px-12 xl:py-5 bg-[#FF2A5F] hover:bg-rose-500 text-white font-bold rounded-full text-[10px] md:text-xs xl:text-sm tracking-widest shadow-[0_4px_15px_rgba(255,42,95,0.4)] transition-all uppercase whitespace-nowrap">
              JOIN NOW
            </a>
            <a href="#about" className="cursor-pointer text-white hover:text-[#FF2A5F] font-bold tracking-wider text-[11px] md:text-xs xl:text-sm transition-colors drop-shadow-md uppercase whitespace-nowrap">
              Explore
            </a>
          </div>
        </div>
